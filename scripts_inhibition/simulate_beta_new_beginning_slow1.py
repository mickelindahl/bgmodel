...
import oscillation_perturbations_new_beginning_slow1 as op_nb
import pprint
pp=pprint.pprint
...
ops=[op.get()[9]]
amp_base=[1.05, 1.1] #numpy.arange(1.05, 1.2, 0.05)
freqs=[ 0.3] #numpy.arange(0.5, .8, 0.2)
n=len(amp_base)
m=len(freqs)
amp_base=list(numpy.array([m*[v] for v in amp_base]).ravel()) 
freqs=list(freqs)*n
STN_amp_mod=[0.]#range(1, 6, 2)
num_runs=len(freqs)*len(STN_amp_mod)*len(ops)*len(op_nb.get())
