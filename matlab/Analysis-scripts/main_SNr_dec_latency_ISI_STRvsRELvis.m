...
function [] = main_SNr_dec_latency_ISI_STRvsRELvis(data_dir,fig_dir)
%     warning('off')
    nc_id = 1;
...
%                 end
                ...
                if isnan(time_diff_mean)
                    time_diff_avg(strf_ind,relt_ind,stnf_ind) = 0;
                else
                    time_diff_avg(strf_ind,relt_ind,stnf_ind) = time_diff_mean;
                end
                time_pos_diff_avg(strf_ind,relt_ind,stnf_ind) = mean(time_diff(~isnan(time_diff) & ...
                                                                              (time_diff>=0)));
                num_non_nan(strf_ind,relt_ind,stnf_ind) = sum(~isnan(time_diff));
