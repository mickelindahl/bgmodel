...
function [] = SNr_dec_latency_ISI_STNGPA_stop_vs_GPA_RELvsRELvis_avgcomp(data_dir,GPA_stop_dir,fig_dir)
%     warning('off')
...
%     GPA_stop_dir = '/space2/mohaghegh-data/Working-Directory/PhD/Projects/BGmodel/MATLAB-data-analysis/Analysis/Gostop-avgfrs-STN-1000-2000-STR-400-2000-reltime--130-20/';
    ...
    fig_dir = [fig_dir,'latency-comparison-ISI-20-vs-GPA-RELvsREL-avgcompare/',nuclei{nc_id},'/'];
...
%                                              sum(~isnan(cnt_time)))/sum(~isnan(cnt_str_time))*100;  %percent
                        ...
                        % Averaging comparisions
                        time_diff = cnt_time - cnt_str_time;
%                         time_diff_mean = mean(time_diff(~isnan(time_diff)));
                        
                        % Comparing averages instead of averaging
                        % comparisions
                        cnt_time_mean = mean(cnt_time(~isnan(cnt_time)));
                        cnt_str_time_mean = mean(cnt_str_time(~isnan(cnt_str_time)));
                        time_diff_mean = cnt_time_mean - cnt_str_time_mean;
...
%                                              %Percent
                        ...
                        % Comparing averages instead of averaging
                        % comparisions
                        cnt_time_mean = mean(cnt_time_gpa(~isnan(cnt_time_gpa)));
                        cnt_str_time_mean = mean(cnt_str_time_gpa(~isnan(cnt_str_time_gpa)));
                        time_diff_mean_stn = cnt_time_mean - cnt_str_time_mean;
...
%                         num_non_nan(rel1_ind,rel2_ind,stnf_ind,strf_ind,gpaf_ind) = sum(~isnan(time_diff));
    ...
    save([fig_dir,'latency-var-peakth','-widthth',num2str(width_th)],...
         'time_diff_avg','time_diff_avg_stn',...
         'num_suppressed_mov','num_suppressed_mov_stn','time_diff_stngpa_stn',...
         'params_stngpa','params_gpa','num_mov_str','num_mov','num_mov_str_gpa','num_mov_gpa')
end
