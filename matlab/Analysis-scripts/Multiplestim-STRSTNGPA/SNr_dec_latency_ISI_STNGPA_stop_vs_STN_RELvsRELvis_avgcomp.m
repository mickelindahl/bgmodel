...
function [] = SNr_dec_latency_ISI_STNGPA_stop_vs_STN_RELvsRELvis_avgcomp(data_dir,STN_stop_dir,fig_dir)
%     warning('off')
...
%     STN_stop_dir = '/space2/mohaghegh-data/Working-Directory/PhD/Projects/BGmodel/MATLAB-data-analysis/Analysis/Gostop-avgfrs-STN-1000-2000-STR-400-2000-reltime--130-20/';
    ...
    fig_dir = [fig_dir,'latency-comparison-ISI-20-vs-STN-RELvsREL-avgcompare/',nuclei{nc_id},'/'];
...
%                                              Percent
                        ...
                        cnt_time_mean = mean(cnt_time_stn(~isnan(cnt_time_stn)));
                        cnt_str_time_mean = mean(cnt_str_time_stn(~isnan(cnt_str_time_stn)));
...
%                     '-STR = ',num2str(str_f(strf_ind),'%i'),' (Hz)'])
        ...
        %         figure;
                ...
                subplot(224)
                imagesc(rel_time_stn,rel_time_gpa,time_diff_avg_stn(:,:,stnf_ind,strf_ind,gpaf_ind))
                GCA = gca;
                GCA.FontSize = 14;
                GCA.Box = 'off';
                GCA.TickDir = 'out';
...
%                                                 [-max(tr_vec),max(tr_vec)])
                ...
                imagesc(rel_time_stn,rel_time_gpa,num_mov_str(:,:,stnf_ind,strf_ind,gpaf_ind),...
                                                [0,max(tr_vec)])
                ...
                ax.Label.FontSize = 14;
                fig = gcf;
                fig.Position = fig.Position.*[1,1,2,2];
        ...
        save([fig_dir,'latency-var-peakth','-widthth',num2str(width_th)],...
             ...
             'params_stngpa','params_stn','num_mov_str','num_mov','num_mov_str_stn','num_mov_stn')
end
