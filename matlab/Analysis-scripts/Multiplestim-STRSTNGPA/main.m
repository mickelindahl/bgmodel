...
function [] = main(data_path,data_dir,weights,numtrs)
    compress_flag = true;
    if exist([data_path,'/all_proc_data.mat'],'file') ~= 2
        ...
        indir = dir(data_path);
        for indir_ind = 1:length(indir)
            name_str = indir(indir_ind).name;
            mat_flname = '/procdata_avg_ISI.mat';
            logical_state = (indir(indir_ind).isdir && ~strcmpi(name_str,'.') && ~strcmpi(name_str,'..')) && ...
                             exist([data_path,name_str,mat_flname],'file') ~= 2;
            if logical_state
                [st_par,nctr,avgfr,...
                 avgfrnoov,offt,stimISI,...
                 nctrISI,nc] = main_postproc([data_path,'/',data_dir{dir_ind}],weights,numtrs,compress_flag);
