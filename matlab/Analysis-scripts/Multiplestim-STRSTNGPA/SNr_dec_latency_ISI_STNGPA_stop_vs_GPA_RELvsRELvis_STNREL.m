...
function [] = SNr_dec_latency_ISI_STNGPA_stop_vs_GPA_RELvsRELvis_STNREL(data_dir,GPA_stop_dir,fig_dir)
%     warning('off')
...
%     GPA_stop_dir = '/space2/mohaghegh-data/Working-Directory/PhD/Projects/BGmodel/MATLAB-data-analysis/Analysis/Gostop-avgfrs-STN-1000-2000-STR-400-2000-reltime--130-20/';
    ...
    fig_dir = [fig_dir,'latency-comparison-ISI-20-vs-GPA-RELvsREL-RELSTN/',nuclei{nc_id},'/'];
...
%     t_vec = avgfr_data.t_samples;
    ...
    data_gpa = load([GPA_stop_dir,'avg_fr_data_eachtr_ISI']);
    offtime_gpa = data_gpa.off_time;
    offtime_g_str = data_gpa.off_time_str;
    params_gpa = data_gpa.params;
...
%     t_vec = t_vec(t_vec >= start_search_time);
    ...
    comb_rel = combvec(rel_time_stn',rel_time_gpa');
    rel_reltime = unique(comb_rel(2,:) - comb_rel(1,:));
...
%     SNr_fr_th = 2;                  % Threshold for the SNr movement trigger
    ...
    %                                              sum(~isnan(cnt_time)))/sum(~isnan(cnt_str_time))*100;  %percent
                            ...
                            sel_ind_gpa = find(params_gpa(:,1) == gpa_f(gpaf_ind) & ...
                                               params_gpa(:,3) == rel_time_gpa(rel1_ind) & ...
                                               params_gpa(:,5) == str_f(strf_ind));

                            cnt_time_gpa = offtime_gpa(sel_ind_gpa);
                            cnt_str_time_gpa = offtime_g_str(sel_ind_gpa);

                            num_mov_str_gpa(rel1_ind,rel2_ind,stnf_ind,strf_ind,gpaf_ind) = sum(~isnan(cnt_str_time_gpa));
                            num_mov_gpa(rel1_ind,rel2_ind,stnf_ind,strf_ind,gpaf_ind) = sum(~isnan(cnt_time_gpa));
    ...
    %                                              %Percent
                            num_suppressed_mov_stn(rel1_ind,rel2_ind,stnf_ind,strf_ind,gpaf_ind) = sum(~isnan(cnt_str_time_gpa)) - ...
                                                 sum(~isnan(cnt_time_gpa));

                            time_diff_stn = cnt_time_gpa - cnt_str_time_gpa;
                            time_diff_mean_stn = mean(time_diff_stn(~isnan(time_diff_stn)));
                            ...
                            if isnan(time_diff_mean_stn)
                                time_diff_avg_stn(rel1_ind,rel2_ind,stnf_ind,strf_ind,gpaf_ind) = 0;
                            else
                                time_diff_avg_stn(rel1_ind,rel2_ind,stnf_ind,strf_ind,gpaf_ind) = time_diff_mean_stn;
                            end
                            ...
                            time_diff_avg(rel1_ind,rel2_ind,stnf_ind,strf_ind,gpaf_ind) = NaN;
                            time_diff_avg_stn(rel1_ind,rel2_ind,stnf_ind,strf_ind,gpaf_ind) = NaN;
                            num_suppressed_mov(rel1_ind,rel2_ind,stnf_ind,strf_ind,gpaf_ind) = NaN;
                            num_suppressed_mov_stn(rel1_ind,rel2_ind,stnf_ind,strf_ind,gpaf_ind) = NaN;
                        end
                ...
                figure;
                subplot(211)
                imagesc(rel_time_stn,rel_reltime,time_diff_avg(:,:,stnf_ind,strf_ind,gpaf_ind) ...
        ...
        %         figure;
                subplot(212)
                imagesc(rel_time_stn,rel_reltime,num_suppressed_mov(:,:,stnf_ind,strf_ind,gpaf_ind)...
        ...
        save([fig_dir,'latency-var-peakth','-widthth',num2str(width_th)],...
             'time_diff_avg','time_diff_avg_stn','num_non_nan',...
             'num_suppressed_mov','num_suppressed_mov_stn','time_diff_stngpa_stn',...
             'params_stngpa','params_gpa','num_mov_str','num_mov','num_mov_str_gpa','num_mov_gpa','rel_reltime')
end
