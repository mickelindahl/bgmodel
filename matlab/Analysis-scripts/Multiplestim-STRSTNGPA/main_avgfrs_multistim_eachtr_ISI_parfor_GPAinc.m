...
function [] = main_avgfrs_multistim_eachtr_ISI_parfor_GPAinc(data_dir,res_dir,disinh_width)
    if isempty(data_dir)
...
%         num_samples = length(t_samples);
        for gpaf_ind = 1:length(gpa_f)
           ...
           spk_times_sel = spk_times(spk_times >= l_sim_time - 1000 &...
                                     spk_times <= h_sim_time + 1000);
           trials_gostop_sel = trials_gostop(spk_times >= l_sim_time - 1000 &...
                                             spk_times <= h_sim_time + 1000);
%            stn_f_sel = stn_f(gpa_f == gpa_f(gpaf_ind));
%            str_f_sel = str_f(gpa_f == gpa_f(gpaf_ind));
           stimrate_sel = stimrate(:,stimrate(1,:) == gpa_f(gpaf_ind));
           stimtimes_sel = stimtimes(:,stimrate(1,:) == gpa_f(gpaf_ind));
                                         
           [of_tmp,ofs_tmp,par_tmp] = stn_str_reltimes_func(spk_times_sel,trials_gostop_sel,...
                                 stn_f,str_f,...
                                 stimtimes_sel,stimrate_sel(2:end,:),tr_vec,...
                                 disinh_width,stimtimes_c,stimrate_c,...
                                 spk_times_c,trials_go,gpa_f(gpaf_ind));
                             
            params = [params;par_tmp];
            off_time = [off_time;of_tmp];
            off_time_str = [off_time_str;ofs_tmp];
...
function [off_time,off_time_str,params] = stn_str_reltimes_func(spk_times,trials_gostop,...
                                 stn_f,str_f,...
                                 stimtimes,stimrate,tr_vec,disinh_width,...
                                 stimtimes_c,stimrate_c,spk_times_c,...
                                 trials_go,gpa_F)
...
%                                     stimrate(2,:) == stn_f(stnf_ind));
            ...
            parfor st_id = 1:length(sel_stimtimes)
%                     str_freq = stimrate(3,stimtimes == sel_stimtimes(st_id));
                rel_time = stimrate(3:4,stimtimes == sel_stimtimes(st_id));
%                 disp('Reltimes: ',num2str(rel_time))
