...
%         stn_rel = combvec(stn_f,rel_time);
        ...
        % GPA+STN+STR data load
        
        gpastn_data = load([gpa_stn_data_dir,'/',nuclei{nc_id},'/avg_fr_data_eachtr.mat']);
        
        gpa_cnt_times = gpastn_data.t_samples;
        gpa_params = gpastn_data.params;
%         stn_rel = combvec(stn_f,rel_time);
        gpa_cnt = gpastn_data.cnt;
        gpa_cnt_str = gpastn_data.cnt_str;
        
        gpa_cnt_ind = find(gpa_params(:,1) == gpa_stim & ...
                           gpa_params(:,2) == stn_stim & ...
                           gpa_params(:,3) == str_stim & ...
                           gpa_params(:,4) == rel_time_gpa & ...
                           gpa_params(:,5) == rel_time_stn);
        
        
        gpa_cnt_sel = gpa_cnt(gpa_cnt_ind,:);
                  
        gpa_cnt_str_sel = gpa_cnt_str(gpa_cnt_ind,:);
        ...
        gpa_cnt_sel_baseline = mean(gpa_cnt_sel(time_baseline));
...
%         cnt_str_sel = (cnt_str_sel - mean(cnt_str_sel))/std(cnt_str_sel);
        ...
        if avg_fr_flag == 2
            ...
            %GPA + STN + STR  stim
            gpa_cnt_sel = gpa_cnt_sel - gpa_cnt_sel_baseline;
            gpa_cnt_sel = gpa_cnt_sel/max(abs(cnt_sel));
    ...
    %         cnt_str_sel = cnt_str_sel/(max(cnt_str_sel)-min(cnt_str_sel));
        elseif avg_fr_flag == 1
            ...
            %GPA + STN + STR  stim
            gpa_cnt_sel = gpa_cnt_sel - gpa_cnt_sel_baseline;
            gpa_cnt_sel = gpa_cnt_sel/std(gpa_cnt_sel);
    ...
    %         cnt_str_sel = cnt_str_sel/(max(cnt_str_sel)-min(cnt_str_sel));
        ...
        % Plotting GPA + STN + STR
        
        subplot(3,1,1)
        hold on
        plot(cnt_times,gpa_cnt_sel,'LineStyle',linestyle_vec{nc_id},...
        ...
        title([ 'STR:',num2str(str_stim),...
                '-STN:',num2str(stn_stim),...
                '-GPA:',num2str(gpa_stim),...
                '-RS:',num2str(rel_time_stn),...
                '-RG:',num2str(rel_time_gpa)])
        ...
        GCA.TickDir = 'out';
        ylabel(['Average firing rate'])
        title('Stop signal in STN')
%         title([ 'STR = ',num2str(str_freq(str_f_ind)),' (Hz)',...
...
%                 '-REL = ',num2str(rel_time(rel_t_ind)),' (ms)'])
    ...
    fig_print(gcf,[fig_dir,'avgfr_compare-',nuclei{nc_id},...
        ...
        '-STR',num2str(str_freq(str_f_ind)),...
        '-',trg_name,...
        '-',num2str(10)])
end
