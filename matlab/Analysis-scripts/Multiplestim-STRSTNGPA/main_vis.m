...
% function [] = main_vis()
    ...
    relsg = unique(strstngpa.stim_param(:,5));
    relss = unique(strstngpa.stim_param(:,4));
...
%      off_str,off_stn] = trbytr_delay_measure(str,strstn,strstngpa);
    ...
    nodecinSN(spda_data.no_decrease,numtrs,dir_name)
    ...
    for str_ind = 1:length(str_f)
        delayvis(spda_data.pos_delay_gp,numtrs*length(stn_f)*length(relss),dir_name,str_f(str_ind),'PositiveDelay-GPASTN')
        delayvis(spda_data.neg_delay_gp,numtrs*length(stn_f)*length(relss),dir_name,str_f(str_ind),'NegativeDelay-GPASTN')
    end
    ...
    for str_ind = 1:length(str_f)
        delayvis_stn(spda_data.pos_delay_st,numtrs,dir_name,str_f(str_ind),'PositiveDelay-STN')
        delayvis_stn(spda_data.neg_delay_st,numtrs,dir_name,str_f(str_ind),'NegativeDelay-STN')
    end
...
function [gpa_str_delay,stn_str_delay,gpa_stn_delay] = delay_measure_average(STR,STN,GPA,all_stim_params,Ws)
%     all_stim_par = combvec(str_f',stn_f',gpa_f',relss',relsg');
    ...
    for w_ind = 1:length(Ws)
        for asp_ind = 1:size(all_stim_params,2)
            sel_gpa = GPA.stim_param_ISI(:,2) == all_stim_params(1,asp_ind) & ...
                      ...
                      GPA.stim_param_ISI(:,4) == all_stim_params(4,asp_ind) & ...
                      GPA.stim_param_ISI(:,5) == all_stim_params(5,asp_ind) & ...
        ...
        for w_ind = 1:length(Ws)
            for s_ind = 1:size(stim,1)
                sel = data_in.del_w(:,3) == Ws(w_ind) & data_in.stim_par(:,1) == stim(s_ind,1) & ...
                      data_in.stim_par(:,2) == strf & data_in.stim_par(:,5) == rel(r_ind);
