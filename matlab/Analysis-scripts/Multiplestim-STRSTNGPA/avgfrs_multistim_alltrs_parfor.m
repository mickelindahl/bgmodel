...
function [] = avgfrs_multistim_alltrs_parfor(data_dir,res_dir)
...
%     nuclei = {'SN','FS','GA','M1','M2','ST'};
    nc_names = {'SNr','FSI','GPe Arky','GPe Proto',...
                'MSN D1','MSN D2','STN'};
%     nc_names = {'SNr','FSI','GPe Arky',...
%                 'MSN D1','MSN D2','STN'};
...
%         params = zeros(size(cnt,1),length(params_str));
        ...
        for gpaf_ind = 1:length(gpa_f)
            ...
            for stnf_ind = 1:length(stn_f)
                ...
                parfor rel_time_ind = 1:length(stimtimes_sel_sec)
