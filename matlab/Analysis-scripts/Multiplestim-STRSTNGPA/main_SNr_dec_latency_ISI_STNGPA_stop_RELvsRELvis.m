...
%                                              sum(~isnan(cnt_time)))/sum(~isnan(cnt_str_time))*100;      %In percentage
                        num_suppressed_mov(rel1_ind,rel2_ind,stnf_ind,strf_ind,gpaf_ind) = sum(~isnan(cnt_str_time)) - ...
                                             sum(~isnan(cnt_time));      %In numbers
...
%             end
        ...
        %         figure;
                subplot(212)
                imagesc(rel_time_stn,rel_time_gpa,num_suppressed_mov(:,:,stnf_ind,strf_ind,gpaf_ind),[-num_trials,num_trials])
                GCA = gca;
                GCA.FontSize = 14;
                GCA.Box = 'off';
                GCA.TickDir = 'out';
                ...
                if exclusive_fig_dirs == 1                                
                    ...
                    fig_print(gcf,[STR_fig_dir,'numsamples-',num2str(gpa_f(gpaf_ind),'%i')...
                                              ,num2str(stn_f(stnf_ind),'%i')])

                    fig_print(gcf,[STN_fig_dir,'numsamples-',num2str(stn_f(stnf_ind),'%i')...
                                              ,num2str(str_f(strf_ind),'%i')])
                                          
                    fig_print(gcf,[GPA_fig_dir,'numsamples-',num2str(stn_f(stnf_ind),'%i')...
                                              ,num2str(str_f(strf_ind),'%i')])
