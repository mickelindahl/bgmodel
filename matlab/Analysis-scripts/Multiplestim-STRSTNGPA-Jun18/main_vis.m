...
function [] = main_vis(dir_name)
    ...
    %     dir_name = '/Users/Mohammad/Documents/PhD/Projects/BGmodel/MATLAB-data-analysis/Analysis/18-03-28-separatesims-sensoryinSTNGPA-rampinSTR';
        dir_name = '/Volumes/SpaceMac/BGmodel-files/18-06-14-randominput-fixedconn-reseting';
    ...
    fl_indir = fullfile(dir_name,'AllProcData');
    
    if exist(fl_name,'file') == 2
        tmpdata = load(fl_name);
    
%     strstn = struct_conc(procdata,1,2);
        strstn = tmpdata.procdata{1,1};
        strstn.stim_param(:,[1,4]) = [];
        strstn.stim_param_ISI(:,[1,4]) = [];
%     str = procdata{1,2};
        str = tmpdata.procdata{1,end};
...
%     adding_ind = 4:9;
        base_ind = 2;
        adding_ind = 3:5;
        strstngpa = struct_conc(tmpdata.procdata,base_ind,adding_ind);
%     strstngpa = procdata{1,end};
        time_vec = strstn.time_vec;
        
        clear tmpdata
        
    elseif exist(fl_indir,'dir') == 7
        
        tmpdata = load(fullfile(fl_indir,'all_proc_data-STR.mat'));
        str = tmpdata.procdata;
        
        clear tmpdata
        
        tmpdata = load(fullfile(fl_indir,'all_proc_data-STRSTN.mat'));
        strstn = tmpdata.procdata;
        
        clear tmpdata
        
        tmpdata = load(fullfile(fl_indir,'all_proc_data-STRSTNGPA.mat'));
        strstngpa = tmpdata.procdata;
        ncs = tmpdata.nc;
        ...
        disp('Processed data does not exist!')
    end
...
%      off_str,off_stn] = trbytr_delay_measure(str,strstn,strstngpa);
    ...
    effective_params(spda_data.neg_delay10_gp,delays,[str,strstn,strstngpa],all_stim_par,Ws,numtrs,Figdir,0.5,'EffcNegDelay10-GPAvsSTN')
...
%     save(fullfile(fig_dir,'outputdata'),sig_params)
    ...
    fclose(fileID);
    save(fullfile(fig_dir,'outputdata1'),'sel_params', 'all_del', 'all_supp')
end
