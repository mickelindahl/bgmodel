...
function [] = main_spectrogram_func()
data_dir = [pwd,'/Ramp-dur140-amp200-1000-10/'];%mat_data/'];
fig_dir = [data_dir,'/figs/Spectrum-matlabspectrogram-Freqlimited/'];
...
%         raster_time_id_nest_rand(spktimes_raster,ids_raster,random_sel,'blue')
        ...
        hamm_filt_win = hamming(win_width);
        cnt_f = conv(cnt,hamm_filt_win,'same');
% 
...
%         Spec = spectrogram_int_freqs(cnt_f,Ker);
        ...
        spec_win = 100;
        [Spec,Freq,Time,Pow] = spectrogram(cnt_f,spec_win,spec_win-1,[],fs);
        spectimes = cnttimes - stimtimes(st_id);
        time_vec = linspace(min(spectimes),max(spectimes),length(Time));
        tim_lim = time_vec >= -500 & time_vec <= 500;
        fr_lim = Freq >= 0;
        
%         figure();
%         imagesc(log10(Pow),[-2,0])
%         fr_lim = frs >= 10;
%         tim_lim = spectimes >= -0.5 & spectimes <= 0.5;
        
%         imagesc(spectimes(tim_lim),frs(fr_lim),log10(Spec(fr_lim,tim_lim).^2),[0,3])
        imagesc(time_vec(tim_lim),Freq(fr_lim),log10(Pow(fr_lim,tim_lim)))
        GCA = gca;
        GCA.FontSize = 14;
        GCA.Box = 'off';
        GCA.TickDir = 'out';
        title(nuclei{nc_id})
        ax = colorbar();
        fig_print(gcf,[fig_dir,'Spectrum-ge10Hz-limited-',num2str(win_width),'ms-',nuclei{nc_id},'-',num2str(stimrate(st_id)),...
                        '-',num2str(win_width)])
        close(gcf)
