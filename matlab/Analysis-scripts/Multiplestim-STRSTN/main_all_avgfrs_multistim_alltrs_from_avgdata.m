...
%     end
    ...
    for nc_id = 1:length(nuclei)
        ...
        fig_dir = [res_dir,'/',nuclei{nc_id},'/reltime/'];
        if exist(fig_dir,'dir') ~= 7
            mkdir(fig_dir)
        end
        ...
        variable_comb = combvec(stn_f,str_freq);
...
%                           cnt_times <= 250,:,:);
        ...
        for s_r_ind = 1:length(variable_comb)
            stn_f_ind = find(stn_f == variable_comb(1,s_r_ind));
%             rel_t_ind = find(rel_time == variable_comb(2,s_r_ind));
            str_f_ind = find(str_freq == variable_comb(2,s_r_ind));
            
            image_mat = cnt(str_f_ind,:,:,stn_f_ind);
            image_mat = reshape(image_mat,[size(image_mat,2),size(image_mat,3)]);
            image_mat = image_mat';
...
%                      max(max(cnt(:,:,rel_t_ind,stn_f_ind)))])
            imagesc(cnt_times,rel_time,image_mat,...
                    [min(image_mat(:)),max(image_mat(:))])
            xlim([-150,250])
            GCA = gca;
            GCA.FontSize = 14;
            GCA.Box = 'off';
...
%                 '-STR = ',num2str(str_freq(rel_t_ind)),' (ms)'])
            title([nc_names{nc_id},...
                '-STN = ',num2str(stn_f(stn_f_ind)),' (Hz)',...
                '-STR = ',num2str(str_freq(str_f_ind)),' (ms)'])
            ax = colorbar();
...
%                      max(max(cnt(:,:,rel_t_ind,stn_f_ind)))])
            ...
            image_mat = cnt_str(str_f_ind,:,:,stn_f_ind);
            image_mat = reshape(image_mat,[size(image_mat,2),size(image_mat,3)]);
            image_mat = image_mat';
