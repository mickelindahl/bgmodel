...
function [] = main_all_avgfrs_multistim_GPA_alltrs_from_avgfrdata(data_dir,res_dir)
    if isempty(data_dir)
    ...
    for nc_id = 1:1%length(nuclei)
        ...
        GSRR = combvec(gpa_f',str_freq',rel_str_gpe',rel_str_stn');
        cnt = data.cnt;
...
%                           cnt_times <= 250,:,:);
        ...
        for s_r_ind = 1:length(GSRR)
            sel_ind = find(params(:,1) == GSRR(1,s_r_ind) & ...
                           params(:,3) == GSRR(2,s_r_ind) & ...
                           params(:,4) == GSRR(3,s_r_ind) & ...
                           params(:,5) == GSRR(4,s_r_ind));
        
            figure;
            subplot(211)
            imagesc(cnt_times,stn_f,cnt(sel_ind,:),...
                    [min(cnt(:)),...
                     max(cnt(:))])
            xlim([-150,250])
            GCA = gca;
...
%                 xlabel('Time to ramp offset (ms)')
            ...
            title([nc_names{nc_id},...
                '-GPA = ',num2str(GSRR(1,s_r_ind),'%i'),' (Hz)',...
                '-STR = ',num2str(GSRR(2,s_r_ind),'%i'),' (Hz)',...
                '-RELSG = ',num2str(GSRR(3,s_r_ind),'%i'),' (ms)',...
                '-RELSS = ',num2str(GSRR(4,s_r_ind),'%i'),' (ms)'])
            ax = colorbar();
