...
function [] = main_all_avgfrs_multistim(data_dir,data_dir_compare)
        ...
        for stnf_ind = 1:length(stn_f)
            for rel_time_ind = 1:length(str_stn_lat)
                ...
                for st_id = 1:length(sel_stimtimes)
                    ...
                    st_time = sel_stimtimes_c(st_id) - 500;
                    end_time = sel_stimtimes_c(st_id) + 500;
                    [cnt_str(st_id,:),cnttimes_str] = PSTH_mov_win(spk_times,win_width,overlap,st_time,end_time,numunits,1);
            %         figure;
            %         
            %         subplot(211)
            ...
            %         pause()
                ...
                diff_frs = mean(cnt - cnt_str,2); % The differece is calculated to
