...
%     end
    ...
    for st_id = 1:1:length(stimtimes)
        disp([nc_names{1},'-',num2str(st_id)])
        st_time = stimtimes(st_id) - 1000;
        end_time = stimtimes(st_id) + 1000;
        [M1cnt(st_id,:),M1cnttimes] = PSTH_mov_win(M1spk_times,win_width,overlap,st_time,end_time,numunits,1);
        [M2cnt(st_id,:),~] = PSTH_mov_win(M2spk_times,win_width,overlap,st_time,end_time,numunits,1);
%         figure;
%         
%         subplot(211)
%         cnt = (M1cnt + M2cnt)/2;
...
%     fig_print(gcf,'subpop output rate vs. input rate ')
    cnt = (M1cnt + M2cnt)/2;
    figure;
    imagesc(M1cnttimes - stimtimes(st_id),stimrate,cnt)
    GCA = gca;
    GCA.FontSize = 14;
    GCA.Box = 'off';
    GCA.TickDir = 'out';
    ...
    title(['subpopulation - ',nc_names{1}])
    ax = colorbar();
    ax.Label.String = 'Average firing rate (Hz)';
    ax.Label.FontSize = 14;
    fig_print(gcf,[fig_dir,'colorplot-subpop-',nuclei{1}])
    close(gcf)    
end
