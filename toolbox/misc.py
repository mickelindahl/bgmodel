...
def convolve(binned_data, bin_extent, kernel_type, axis=0, single=False,
             std_ms=1, sampling_window=1, no_mean=False, params={}):
...
def dict_recursive_update(dic, keys, val, op='=='):
        ...
        if len(keys)==0:
            ...
            if op=='*':  
                return dic*val
            if op=='+':  return dic+val
        
        if keys[0] in dic.keys():
            dic[keys[0]]=dict_recursive_update(dic[keys[0]], keys[1:], val, op )
        else:
