...
from toolbox.network.data_processing import Data_unit, Data_units_dic, Data_units_relation_dic
...
class Network_model(object):            
    ...
    def __init__(self, label, **kwargs):
        ...
        data_call_default=['firing_rate','firing_rate_sets', 'ids', 'isis',
                           'mean_rates', 'rasters','rasters_sets']
        data_call=kwargs.get('data_call', data_call_default)
        self.data_raw=dict(zip(data_call, [[] for _ in len(data_call)])) #Will be a dict with key for each data call
        self.data=dict(zip(data_call, [None for _ in len(data_call)]))
        ...
        class_network_construction=kwargs.get('class_network_construction', Inhibition)  
    ...
    def get_data_unit_dic(self, models):
        ...
        for call in self.data_call:
            # Set data for data units for each data call
            func=getattr(dud, 'set_'+call)
            self.data[call]=func(self.data_raw[call])   
    ...
    def simulate(self, model_list, print_time=True, **kwargs): 
        ...
        network.inputs()
        network.build()
        #network.randomize_params(['V_m', 'C_m', 'V_th'])
        network.connect()
        ...
        for call in self.data_call:
            func=getattr(network, 'get_'+call)
            self.data_raw[call].append(func(model_list))   
...
class TestNetwork_mode_dic(unittest.TestCase):
    ...
    def setUp(self):     
        ...
        network_class=[Inhibition, Slow_wave,  Bcpnn_h0, Bcpnn_h1]
