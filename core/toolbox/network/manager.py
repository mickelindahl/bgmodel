...
from toolbox.network.default_params import Par_base, Inhibition
...
class Director_networks(object):
...
class Builder_network(Builder_abstract):    
...
def grid(start, stop, res):
    
    
    x, y=numpy.mgrid[0:res,0:res]
    x, y=x.ravel(), y.ravel()    
    x = x*stop+start
    
class Builder_burst_compete(Builder_network):    
...
def load(file_name, keys, *args):
    return Storage_dic.load_dic(file_name, keys, *args)
