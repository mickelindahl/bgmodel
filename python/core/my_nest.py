...
if hasattr(nest, "version"):
    ver = float(nest.version().split(' ')[1])
else:
    ver = float(nest.__version__)
...
def _Connect_speed_internal(pre, post, params=None, delay=None, model="static_synapse"):
    ...
    # pre post w d Connect
        ...
        if len(params) != len(pre):
            raise NESTError(
                "params must be a float, or list of floats of length 1 or len(pre) and will be used as weight(s).")
...
def collect_spikes_mpi(*args):
    ...
    for i in range(len(args)):
        with Barrier():
            if comm.rank() == 0:
                for i_proc in range(1, comm.size()):
...
def sim_group(data_path, **kwargs):
    ...
    # nest.RandomConvergentConnect(pg, n, 100)
