...
def connect(popus, surfs, params_nest, params_conn, display_print=False, with_out_pre_calculation=False):
    for source in popus:
    ...
    if not with_out_pre_calculation:
        ...
        connect_conns_without_pre_calculation(popus, surfs, params_nest, params_conn,display_print)
...
def connect_conns_without_pre_calculation(popus, surfs, params_nest, params_conn, display_print=False):

        for val in params_conn.values():
            my_nest.MyCopyModel(params_nest[val['syn']], val['syn'])

        for key, val in params_conn:

            if val['lesion']:
                continue

            conn = {'rule': 'fixed_indegree', 'indegree': val['fan_in'],
                    'autapses': False, 'multapses': False}

            source = popus[val['source']]
            target = popus[val['target']]
            nest.Connect(source, target, conn, val['syn'])
