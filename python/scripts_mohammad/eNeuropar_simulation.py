...
def main():
    ...
    nvprocs = nest.GetKernelStatus('total_num_virtual_procs')
    randgens = [numpy.random.RandomState(s) for s in range(0,nvprocs)]
    ...
    '''
    hets = params_dic['node']['M1']['rand']
    obj_stat = nest.GetStatus(M1)
    for het_par in hets.keys():
        if hets[het_par]['active']:
            for n_id in range(0,len(obj_stat)):
                vp = obj_stat[n_id]['vp']
                mod_params = numpy.round(heterogeneity(het_par,hets,vp,randgens),2)
                #mod_params = heterogeneity(het_par,hets,vp,randgens)
                #print het_par,n_id,mod_params
                nest.SetStatus([M1[n_id]],{het_par:mod_params})
    '''
    ...
    '''
    hets = params_dic['node']['M2']['rand']
    obj_stat = nest.GetStatus(M2)
    for het_par in hets.keys():
        if hets[het_par]['active']:
            for n_id in range(0,len(obj_stat)):
                vp = obj_stat[n_id]['vp']
    ...
    FS_params['I_e'] = params_dic['node']['FS']['nest_params']['I_e']

    nest.CopyModel(FS_model, 'FSI', params = FS_params)

    FS = nest.Create('FSI', FS_n)
    '''
    hets = params_dic['node']['FS']['rand']
    obj_stat = nest.GetStatus(FS)
    for het_par in hets.keys():
        if hets[het_par]['active']:
            for n_id in range(0,len(obj_stat)):
                vp = obj_stat[n_id]['vp']
    ...
    ST_params['I_e'] = params_dic['node']['ST']['nest_params']['I_e']

    nest.CopyModel(ST_model, 'STN', params = ST_params)

    ST = nest.Create('STN', ST_n)
    '''
    hets = params_dic['node']['ST']['rand']
    obj_stat = nest.GetStatus(ST)
    for het_par in hets.keys():
        if hets[het_par]['active']:
            for n_id in range(0,len(obj_stat)):
                vp = obj_stat[n_id]['vp']
    ...
    GA_params['I_e'] = params_dic['node']['GA']['nest_params']['I_e']

    nest.CopyModel(GA_model, 'GPArky', params = GA_params)

    GA = nest.Create('GPArky', GA_n)
    '''
    hets = params_dic['node']['GA']['rand']
    obj_stat = nest.GetStatus(GA)
    for het_par in hets.keys():
        if hets[het_par]['active']:
            for n_id in range(0,len(obj_stat)):
                vp = obj_stat[n_id]['vp']
    ...
    GI_params['I_e'] = params_dic['node']['GI']['nest_params']['I_e']

    nest.CopyModel(GI_model, 'GPProto', params = GI_params)

    GI = nest.Create('GPProto', GI_n)
    '''
    hets = params_dic['node']['GI']['rand']
    obj_stat = nest.GetStatus(GI)
    for het_par in hets.keys():
        if hets[het_par]['active']:
            for n_id in range(0,len(obj_stat)):
                vp = obj_stat[n_id]['vp']
    ...
    SN_params['I_e'] = params_dic['node']['SN']['nest_params']['I_e']

    nest.CopyModel(SN_model, 'SNr', params = SN_params)

    SN = nest.Create('SNr', SN_n)
    '''
    hets = params_dic['node']['SN']['rand']
    obj_stat = nest.GetStatus(SN)
    for het_par in hets.keys():
        if hets[het_par]['active']:
            for n_id in range(0,len(obj_stat)):
                vp = obj_stat[n_id]['vp']
    ...
    GF_params['I_e'] = params_dic['node']['GF']['nest_params']['I_e']

    nest.CopyModel(GF_model, 'GPFS', params = GF_params)

    GF = nest.Create('GPFS', GF_n)
    '''
    hets = params_dic['node']['GF']['rand']
    obj_stat = nest.GetStatus(GF)
    for het_par in hets.keys():
        if hets[het_par]['active']:
            for n_id in range(0,len(obj_stat)):
                vp = obj_stat[n_id]['vp']
    ...
    if not isconn:
        ...
        syndic['delay'] = conn_heterogeneity('delay',conn_het_dic)
        syndic['weight'] = conn_heterogeneity('weight',conn_het_dic)
        syndic['model'] = syn
    ...
    if not isconn:
        ...
        syndic['model'] = syn
        syndic.pop('receptor_type')
        nest.Connect(CF, FS, syn_spec=syndic)
    ...
    if not isconn:
        ...
        syndic['model'] = syn
        syndic.pop('receptor_type')
        nest.Connect(CS, ST, syn_spec=syndic)
    ...
    if not isconn:
        ...
        syndic['model'] = syn
        syndic.pop('receptor_type')
        nest.Connect(CSN, ST, syn_spec=syndic)
    ...
    if not isconn:
        ...
        syndic['model'] = syn
        syndic.pop('receptor_type')
        nest.Connect(ES, SN, syn_spec=syndic)
    ...
    if not isconn:
        ...
        syndic['model'] = syn
        syndic.pop('receptor_type')
        nest.Connect(EA, GA, syn_spec=syndic)
    ...
    if not isconn:
        ...
        syndic['model'] = syn
        syndic.pop('receptor_type')
        nest.Connect(EI, GI, syn_spec=syndic)
    ...
    if not isconn:
        ...
        syndic['model'] = syn
        syndic.pop('receptor_type')
        nest.Connect(EF, GF, syn_spec=syndic)
    ...
    if not isconn:
        ...
        syndic['model'] = syn
        syndic.pop('receptor_type')
        nest.Connect(M1, M1, conn_spec=conn, syn_spec=syndic)
    ...
    if not isconn:
        ...
        syndic['model'] = syn
        syndic.pop('receptor_type')
        nest.Connect(M1, M2, conn_spec=conn, syn_spec=syndic)
    ...
    if not isconn:
        ...
        syndic['model'] = syn
        syndic.pop('receptor_type')
        nest.Connect(M2, M2, conn_spec=conn, syn_spec=syndic)
    ...
    if not isconn:
        ...
        syndic['model'] = syn
        syndic.pop('receptor_type')
        nest.Connect(M2, M1, conn_spec=conn, syn_spec=syndic)
    ...
    if not isconn:
        ...
        syndic['model'] = syn
        syndic.pop('receptor_type')
        nest.Connect(FS, M1, conn_spec=conn, syn_spec=syndic)
    ...
    if not isconn:
        ...
        syndic['model'] = syn
        syndic.pop('receptor_type')
        nest.Connect(FS, M2, conn_spec=conn, syn_spec=syndic)
    ...
    if not isconn:
        ...
        syndic['model'] = syn
        syndic.pop('receptor_type')
        nest.Connect(FS, FS, conn_spec=conn, syn_spec=syndic)
    ...
    if not isconn:
        ...
        syndic['model'] = syn
        syndic.pop('receptor_type')
        nest.Connect(M1, SN, conn_spec=conn, syn_spec=syndic)
    ...
    if not isconn:
        ...
        syndic['model'] = syn
        syndic.pop('receptor_type')
        nest.Connect(M2, GA, conn_spec=conn, syn_spec=syndic)
    ...
    if not isconn:
        ...
        syndic['model'] = syn
        syndic.pop('receptor_type')
        nest.Connect(GA, M2, conn_spec=conn, syn_spec=syndic)
    ...
    if not isconn:
        ...
        syndic['model'] = syn
        syndic.pop('receptor_type')
        nest.Connect(GA, M1, conn_spec=conn, syn_spec=syndic)
    ...
    if not isconn:
        ...
        syndic['model'] = syn
        syndic.pop('receptor_type')
        nest.Connect(M2, GI, conn_spec=conn, syn_spec=syndic)
    ...
    if not isconn:
        ...
        syndic['model'] = syn
        syndic.pop('receptor_type')
        nest.Connect(GI, M2, conn_spec=conn, syn_spec=syndic)
    ...
    if not isconn:
        ...
        syndic['model'] = syn
        syndic.pop('receptor_type')
        nest.Connect(GI, M1, conn_spec=conn, syn_spec=syndic)
    ...
    if not isconn:
        ...
        syndic['model'] = syn
        syndic.pop('receptor_type')
        nest.Connect(M2, GF, conn_spec=conn, syn_spec=syndic)
    ...
    if not isconn:
        ...
        syndic['model'] = syn
        syndic.pop('receptor_type')
        nest.Connect(GF, M2, conn_spec=conn, syn_spec=syndic)
    ...
    if not isconn:
        ...
        syndic['model'] = syn
        syndic.pop('receptor_type')
        nest.Connect(GF, M1, conn_spec=conn, syn_spec=syndic)
    ...
    if not isconn:
        ...
        syndic['model'] = syn
        syndic.pop('receptor_type')
        nest.Connect(GF, FS, conn_spec=conn, syn_spec=syndic)
    ...
    if not isconn:
        ...
        syndic['model'] = syn
        syndic.pop('receptor_type')
        nest.Connect(GI, FS, conn_spec=conn, syn_spec=syndic)
    ...
    if not isconn:
        ...
        syndic['model'] = syn
        syndic.pop('receptor_type')
        nest.Connect(GA, FS, conn_spec=conn, syn_spec=syndic)
    ...
    if not isconn:
        ...
        syndic['model'] = syn
        syndic.pop('receptor_type')
        nest.Connect(GF, GI, conn_spec=conn, syn_spec=syndic)
    ...
    if not isconn:
        ...
        syndic['model'] = syn
        syndic.pop('receptor_type')
        nest.Connect(GI, GF, conn_spec=conn, syn_spec=syndic)
    ...
    if not isconn:
        ...
        syndic['model'] = syn
        syndic.pop('receptor_type')
        nest.Connect(GF, GA, conn_spec=conn, syn_spec=syndic)
    ...
    if not isconn:
        ...
        syndic['model'] = syn
        syndic.pop('receptor_type')
        nest.Connect(GA, GF, conn_spec=conn, syn_spec=syndic)
    ...
    if not isconn:
        ...
        syndic['model'] = syn
        syndic.pop('receptor_type')
        nest.Connect(GF, GF, conn_spec=conn, syn_spec=syndic)
    ...
    if not isconn:
        ...
        syndic['model'] = syn
        syndic.pop('receptor_type')
        nest.Connect(GA, GA, conn_spec=conn, syn_spec=syndic)
    ...
    if not isconn:
        ...
        syndic['model'] = syn
        syndic.pop('receptor_type')
        nest.Connect(GI, GI, conn_spec=conn, syn_spec=syndic)
    ...
    if not isconn:
        ...
        syndic['model'] = syn
        syndic.pop('receptor_type')
        nest.Connect(GI, GA, conn_spec=conn, syn_spec=syndic)
    ...
    if not isconn:
        ...
        syndic['model'] = syn
        syndic.pop('receptor_type')
        nest.Connect(GA, GI, conn_spec=conn, syn_spec=syndic)
    ...
    if not isconn:
        ...
        syndic['model'] = syn
        syndic.pop('receptor_type')
        nest.Connect(GI, ST, conn_spec=conn, syn_spec=syndic)
    ...
    if not isconn:
        ...
        syndic['model'] = syn
        syndic.pop('receptor_type')
        nest.Connect(ST, GI, conn_spec=conn, syn_spec=syndic)
    ...
    if not isconn:
        ...
        syndic['model'] = syn
        syndic.pop('receptor_type')
        nest.Connect(GF, ST, conn_spec=conn, syn_spec=syndic)
    ...
    if not isconn:
        ...
        syndic['model'] = syn
        syndic.pop('receptor_type')
        nest.Connect(ST, GF, conn_spec=conn, syn_spec=syndic)
    ...
    if not isconn:
        ...
        syndic['model'] = syn
        syndic.pop('receptor_type')
        nest.Connect(ST, GA, conn_spec=conn, syn_spec=syndic)
    ...
    if not isconn:
        ...
        syndic['model'] = syn
        syndic.pop('receptor_type')
        nest.Connect(ST, ST, conn_spec=conn, syn_spec=syndic)
    ...
    if not isconn:
        ...
        syndic['model'] = syn
        syndic.pop('receptor_type')
        nest.Connect(ST, SN, conn_spec=conn, syn_spec=syndic)
    ...
    if not isconn:
        ...
        syndic['model'] = syn
        syndic.pop('receptor_type')
        nest.Connect(GF, SN, conn_spec=conn, syn_spec=syndic)
    ...
    if not isconn:
        ...
        syndic['model'] = syn
        syndic.pop('receptor_type')
        nest.Connect(GI, SN, conn_spec=conn, syn_spec=syndic)
...
def heterogeneity(het_par,hets,vp,randgens):
    if 'gaussian' in hets[het_par].keys():
        par=hets[het_par]['gaussian']
        if par['my']==0:
            return 0.0

        else:
            r=randgens[vp].normal(loc=par['my'], scale=par['sigma'])
#         print par.keys()
        if 'cut' in par.keys():
    ...
    if 'uniform' in hets[het_par].keys():
        par=hets[het_par]['uniform']
        return randgens[vp].uniform(par['min'], par['max'])

def conn_heterogeneity(type,conn_het_dic):
    if conn_het_dic[type]['type'] != 'constant' and conn_het_dic[type]['params']['max'] > 0:
        mod_dic = {'distribution':conn_het_dic[type]['type'],
                   'low':conn_het_dic[type]['params']['min'],
                   'high':conn_het_dic[type]['params']['max']}
    else:
        if conn_het_dic[type].has_key('params'):
            mod_dic = conn_het_dic[type]['params']
        else:
            mod_dic = 0
    return mod_dic
