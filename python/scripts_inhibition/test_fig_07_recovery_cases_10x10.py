...
l_op_nuc=[64]#,#[32, 16, 17, 33, 40, 49, 56, 57, 64]#16, 33, 49, 57, 64]
l_op_dop=[]#[5,6]
...
opc=op_conn.get()
if l_op_conn:op_pert_add+=[opc[i] for i in l_op_conn]

opn=op_nuc.get()
if l_op_nuc:op_pert_add+=[opn[i] for i in l_op_nuc]

opn=op_dop.get()
if l_op_dop:op_pert_add+=[opn[i] for i in l_op_dop]
