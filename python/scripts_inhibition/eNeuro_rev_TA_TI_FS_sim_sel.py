...
path_rate_runs=get_path_rate_runs('eNeuro_rev_TA_TI_FS_sim_inh/')
print sys.argv
ops=[op.get()['beta'][int(sys.argv[2])]] if  len(sys.argv)>2 else [op.get()['beta'][0]] #0 is beta

FILE_NAME=__file__.split('/')[-1][0:-3]
FROM_DISK_0=int(sys.argv[1]) if len(sys.argv)>1 else 0
proportion_connected=[0.1, 0.5, 1.]
NUM_RUNS=len(proportion_connected)*len(ops)
NUM_NETS=5
...
kwargs={
        ...
        'do_nets':['Net_1'],#['Net_'+str(i) for i in range(NUM_NETS)], #none means all
...
#         'laptime':1000.0,
        ...
        'nets':['Net_1'], #'Net_{}'.format(i) for i in range(NUM_NETS)],
        'nets_to_run':['Net_1'], # ['Net_{}'.format(i) for i in range(NUM_NETS)],
