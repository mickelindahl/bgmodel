...
for key in sorted(d_sw['data'].keys()):
    v=d_sw['data'][key]['Net_0']['GI_GI']['mean_coherence']
    l.append(v)

l=l[organize['TATIFS']]

ln=list_names_sw[organize['TATIFS']]
...
ax.text(1.4,  0.5, r'$g^{TA\to FS}$, $g_{TI\to FS}$',
        transform=ax.transAxes, va='center', rotation=270)

ax.set_xlabel('Frequency (Hz)')
ax.set_ylabel('TI-TI Coherence')
ax.my_set_no_ticks(xticks=4)
ax.my_set_no_ticks(yticks=4)
ax.set_title('Lesion')
