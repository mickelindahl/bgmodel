...
def show(dstim, d, axs, names, **kw):
    
    for model, ax in zip(kw.get('models', ['IV', 'IF', 'FF']), axs):
        for key, name in zip(sorted(d[model].keys()), names):
            ...
            if model=='IF':
                import numpy
                m=numpy.mean(data.last, axis=1)
                slope=(m[-1]-m[0])/(dstim[model][-1]-dstim[model][0])
                pp({
                    'rates':m,
                    'x':dstim[model],
                    'slope':slope
