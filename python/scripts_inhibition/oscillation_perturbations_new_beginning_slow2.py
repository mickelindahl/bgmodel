...
#         misc.dict_update(d,dd) 
        ...
        dd={'node':{'CS':{'rate': float(a0)},
           ...
           'nest':{'GI_ST_gaba':{'weight':a1 },
                   'ST':{'beta_I_AMPA_1': f_beta_rm(a3),
                         'beta_I_NMDA_1': f_beta_rm(a3)}}}
        
        misc.dict_update(d,dd)
        
        s='CS_{0}_GPST_{1}_GPSTd_{2}_STbI_{3}_pST_{4}_EAr_{5}'.format(a0,str(a1)[0:4],str(a2)[0:4],a3,  name, int(a5))
