...
from python.core.network import default_params
from python.core import my_nest, my_population
from python.core import misc
from python.core.my_population import MyNetworkNode
from python.core.my_signals import DataGeneric, DataIFCurve, DataScatter
import pprint

pp = pprint.pprint

from python.core.network.manager import get_storage_list, save, load
from python.core import directories as dr
from python.core.data_to_disk import Storage_dic

# from python.core import data_to_disk
# import os
...
def get_fig_axs(scale=4):
    ...
    from python.core import plot_settings as ps
...
def simulate_rebound_spike(**kw):
    ...
    for t, c in zip(kw.get('rs_time'), kw.get('rs_curr')):
        ...
        my_nest.Connect(scg[i], [mnn[i]], syn_spec={'receptor_type': rec['CURR']})
        i += 1
...
def simulate_ahp(**kw):
    ...
    sd = {'active': True, 'params': {'record_to': 'memory'}}
    mm = {'active': True, 'params': {'interval': 0.1, 'record_to': 'memory'}}
    p = kw.get('rs_params')
    ...
    for source, target, I in zip(scg, mnn[:], I_vec):
        ...
        my_nest.Connect(source, [target],
                        syn_spec={'receptor_type': rec['CURR']})
    ...
    dg = DataGeneric(**{'x': I_vec, 'y': delays, 'xlabel': 'Time (ms)', 'ylabel': 'Voltage (mV)'})
...
def create_figs(d, file_name_figs, **kw):
    ...
    for i in list(range(4, 10)) + list(range(11, 14)):
...
class TestGP_STN_0csillation(unittest.TestCase):
    def setUp(self):
        ...
        self.net = list(d.keys())[0]

    def test_create_figs(self):
        create_figs(self.d,
                    self.file_name_figs,
                    **{'scale': 4, 'net': self.net})
        pylab.show()
