...
import sys
import oscillation_perturbations38_slow_sw as op
import pprint
pp=pprint.pprint


path_rate_runs=get_path_rate_runs('simulate_inhibition_ZZZ38_slow_sw/')
FILE_NAME=__file__.split('/')[-1][0:-3]
FROM_DISK_0=int(sys.argv[1]) if len(sys.argv)>1 else 2
LOAD_MILNER_ON_SUPERMICRO=False

NUM_NETS=2
amp_base=[0.8, 0.85]#numpy.arange(.85, 1.0, 0.05)
freqs=[0.4, 0.6]#numpy.arange(0.5, .8, 0.2)
