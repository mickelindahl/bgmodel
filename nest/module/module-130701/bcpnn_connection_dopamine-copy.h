...
inline
void BCPNNDopaConnection::progress_state_variables(nest::double_t& t_spike,
		std::vector<nest::double_t>& post_spiketimes) {
	...
	nest::int_t number_iterations = (nest::int_t)(
			(((t_spike - t_last_update_) / resolution)));
	nest::int_t j_flag = 0;
	nest::int_t j_counter = 1;

	//for (nest::int_t timestep = 0; timestep < number_iterations; timestep++) {
	for (nest::int_t t = t_last_update_; t < t_spike; t=t+resolution) {

		if ((dopa_spikes_idx_ < dopa_spikes.size()) &&
				(resolution == dopa_spikes[dopa_spikes_idx_].spike_time_ )-t){
		...
		if ((resolution==(post_spiketimes.at(counter) - t)
