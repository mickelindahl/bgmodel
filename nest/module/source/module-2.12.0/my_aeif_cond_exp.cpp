...
void mynest::my_aeif_cond_exp::Parameters_::get(DictionaryDatum &dd) const
{
    ...
    def<double>(dd, "V_reset_max_slope1",       V_reset_max_slope1);
    def<double>(dd, "V_reset_max_slope2",       V_reset_max_slope2);

	def<double>(dd, "AMPA_1_E_rev",         AMPA_1_E_rev);
	def<double>(dd, "AMPA_1_Tau_decay",     AMPA_1_Tau_decay);

	def<double>(dd, "AMPA_2_E_rev",         AMPA_2_E_rev);
	def<double>(dd, "AMPA_2_Tau_decay",     AMPA_2_Tau_decay);
	...
	def<double>(dd, "beta_I_AMPA_2",		beta_I_AMPA_2);     //!< Dopamine effect on NMDA_1 current
...
void mynest::my_aeif_cond_exp::Parameters_::set(const DictionaryDatum &dd)
{
	...
	updateValue<double>(dd, "a_1",       a_1);
	updateValue<double>(dd, "a_2",       a_2);
	updateValue<double>(dd, "V_a",       V_a);
	...
	updateValue<double>(dd, "V_reset_max_slope1",   V_reset_max_slope1);
	updateValue<double>(dd, "V_reset_max_slope2",   V_reset_max_slope2);

    updateValue<double>(dd, "AMPA_1_E_rev",        AMPA_1_E_rev);
	updateValue<double>(dd, "AMPA_1_Tau_decay",    AMPA_1_Tau_decay);
    updateValue<double>(dd, "AMPA_2_E_rev",        AMPA_2_E_rev);
	updateValue<double>(dd, "AMPA_2_Tau_decay",    AMPA_2_Tau_decay);
	...
	updateValue<double>(dd, "beta_I_AMPA_2",		beta_I_AMPA_2);     //!< Dopamine effect on NMDA_1 current
...
void mynest::my_aeif_cond_exp::calibrate()
{
	...
	//double t=nest::Time::ms(P_.t_ref_);
...
void mynest::my_aeif_cond_exp::update(const nest::Time &origin, const long from, const long to)
{
	assert (
		...
		while ( t < B_.step_ )
		{
			...
			*/
			else if ( S_.y[State_::V_M] >= P_.V_peak_ )
			{
				...
				if ( S_.y[State_::u] < 0 )
					S_.y[State_::V_M] =std::min<double>(P_.V_reset_+ S_.y[State_::u]*P_.V_reset_slope1, P_.V_reset_max_slope1);

				else
					S_.y[State_::V_M] =std::min<double>(P_.V_reset_+ S_.y[State_::u]*P_.V_reset_slope2, P_.V_reset_max_slope2);
